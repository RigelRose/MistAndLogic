...
WITH "CTE_JoinTable" AS (SELECT "CDPOS"."TABKEY"                                                   AS "JoinID",
    ...
    "CTE_Releases" AS (SELECT "JoinTable"."JoinID"                                                                               AS "ReleasesID",
                              ...
                              "JoinTable"."JoinID"                                                                               AS "SalesOrderItemID",
                              ROW_NUMBER()
    ...
    "CTE_BlockFromChangelog" AS (SELECT "JoinTable"."JoinID"                   AS "BlockID",
                                        ...
                                        "JoinTable"."JoinID"                   AS "SalesOrderItemID"
                                 FROM "CTE_JoinTable" AS "JoinTable"
                                 WHERE ("JoinTable"."VALUE_OLD" IS NULL AND "JoinTable"."VALUE_NEW" IS NOT NULL)),
    "CTE_ImplicitActiveBlock" AS (SELECT "VBAP"."MANDT" || "VBAP"."VBELN" || "VBAP"."POSNR" AS "BlockID",
                                         CAST("VBAP"."ERDAT" AS DATE) + CAST(TIMESTAMPDIFF(SECOND, CAST("VBAP"."ERZET" AS DATE),
                                            "VBAP"."ERZET") AS INTERVAL SECOND)             AS "BlockTime",
                                         'BillingBlock'                                     AS "BlockType",
                                         'Block to ' || "VBAP"."FAKSP"                      AS "ValueType",
                                         "VBAP"."FAKSP"                                     AS "VALUE_NEW",
                                         NULL                                               AS "VALUE_OLD",
                                         ...
                                         "VBAP"."MANDT" || "VBAP"."VBELN" || "VBAP"."POSNR" AS "SalesOrderItemID"
                                  FROM "VBAP" AS "VBAP"
                                       LEFT JOIN "CTE_BlockFromChangelog" AS "Block"
                                                 ON "Block"."BlockID" = "VBAP"."MANDT" || "VBAP"."VBELN" || "VBAP"."POSNR"
                                       LEFT JOIN "CTE_Releases" AS "Releases"
                                                 ON "Releases"."ReleasesID"
                                                    = "VBAP"."MANDT" || "VBAP"."VBELN" || "VBAP"."POSNR"
                                       LEFT JOIN "CTE_Changes" AS "Changes"
                                                 ON "Changes"."ID" = "VBAP"."MANDT" || "VBAP"."VBELN" || "VBAP"."POSNR"
                                       LEFT JOIN "USR02" AS "USR02"
    ...
    "CTE_BlockNoRN"
        AS (SELECT "ImplicitActiveBlockNoDuplicates"."BlockID"            AS "BlockID",
                   ...
                   "ImplicitActiveBlockNoDuplicates"."SalesOrderItemID"   AS "SalesOrderItemID"
            ...
            SELECT "ImplicitInactiveBlock"."BlockID"            AS "BlockID",
                   ...
                   "ImplicitInactiveBlock"."SalesOrderItemID"   AS "SalesOrderItemID"
            ...
            SELECT "CTE_BlockFromChangelog"."BlockID"            AS "BlockID",
                   ...
                   "CTE_BlockFromChangelog"."SalesOrderItemID"   AS "SalesOrderItemID"
    ...
    "CTE_Block" AS (SELECT *,
                    ...
                    FROM "CTE_BlockNoRN")
SELECT <%=sourceSystem%>  || 'SalesOrderItemBlock_' || "Block"."BlockID" || '_' || "Block"."SystemBlockNumber" || '_'
       ...
       CASE
           ...
           ELSE COALESCE("VBAP"."FAKSP", 'Data cut - no entry in VBAP')
	...
	<%=sourceSystem%>  || 'SalesOrderItem_' || "Block"."SalesOrderItemID" AS "SalesOrderItem",
...
FROM "CTE_Block" AS "Block"
     ...
     LEFT JOIN "TVFST" AS "TVFST"
               ON "VBAP"."MANDT" = "TVFST"."MANDT"
                   AND "VBAP"."FAKSP" = "TVFST"."FAKSP"
...
WITH "CTE_JoinTable" AS (SELECT "CDPOS"."TABKEY"    AS "TABKEY",
                        ...
                        FROM "CDPOS" AS "CDPOS"
                                 ...
                                 LEFT JOIN "USR02" AS "USR02"
                                           ON "CDHDR"."MANDANT" = "USR02"."MANDT"
                                               AND "CDHDR"."USERNAME" = "USR02"."BNAME"),
    "CTE_BlockFromChangelog" AS (SELECT 'SalesOrderItemBlock_' || "JoinTable"."TABKEY"  AS "BlockID",
                                        ...
                                        CASE
                                            ...
                                            ELSE 'Manual' END                           AS "BlockExecutionType",
                                        "JoinTable"."TCODE"                             AS "BlockOperationType",
                                        "JoinTable"."CHANGENR"                          AS "SystemBlockNumber",
                                        'SalesOrderItem_' || "JoinTable"."TABKEY"       AS "SalesOrderItemID",
        ...
        FROM "CTE_JoinTable" AS "JoinTable"
                 LEFT JOIN "CTE_BlockFromChangelog" AS "BlockFromChangelog"
                           ON "JoinTable"."MANDANT" = "BlockFromChangelog"."MANDANT"
                               ...
                               AND 'SalesOrderItemBlock_' || "JoinTable"."TABKEY" = "BlockFromChangelog"."BlockID"
...
SELECT <%=sourceSystem%>  || 'SalesOrderItemBlock_' || "JoinTable"."TABKEY" || '_'
           || COALESCE("BlockFromChangelog"."SystemBlockNumber", '-1') || '_'
           || CASE
                  WHEN "JoinTable"."FNAME" = 'FAKSP' THEN 'BillingBlock'
              END                                                 AS "ObjectID",
